...
def get_mock_user(profile_type):
    if profile_type == "1":
        # High Risk Diabetic
        return UserProfile(
            hba1c_percent=8.5, fasting_glucose_mg_dl=175, post_prandial_glucose_mg_dl=260,
            hypoglycemia_history=False, diabetes_duration_years=15,
            ldl_cholesterol_mg_dl=170, hdl_cholesterol_mg_dl=35, triglycerides_mg_dl=240,
            systolic_bp_mmHg=145, diastolic_bp_mmHg=95,
            eGFR=55, creatinine_mg_dl=1.4, bmi=32, age_years=50, sex="male", height_cm=170, weight_kg=70, activity_level="sedentary", dietary_preference="vegetarian", allergies=[], goal="weight loss"
        )
    else:
        # Healthy / Controlled
        return UserProfile(
